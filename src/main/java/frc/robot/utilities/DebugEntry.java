...
public class DebugEntry<T> {
  ...
  @Nullable private ShuffleboardTab networkTab;
  @Nullable private SimpleWidget networkEntry;
  private Integer localEntry;
  private Consumer<T> localConsumer;
  private final String name;
  private T lastValue;

  public DebugEntry(T defaultValue, String name, @Nullable Subsystem subsystem) {
    this.name = name;
    ...
    if (!Robot.isCompetition && localEntry != null && subsystem != null) {
  ...
  public DebugEntry<T> withPosition(int colIndex, int rowIndex) {
    if (!Robot.isCompetition && networkEntry != null) networkEntry.withPosition(colIndex, rowIndex);
    return this;
  }

  public DebugEntry<T> withSize(int width, int height) {
    if (!Robot.isCompetition && networkEntry != null) networkEntry.withSize(width, height);
    return this;
  }
